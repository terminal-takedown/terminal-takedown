    ...
    constructor() {
        ...
        this.drawGlitchLine = () => {
            for (let i = 0; i < 6; i++) {
                ...
                fill(randomColor);
                rect(Math.random() * innerWidth, Math.random() * innerHeight, Math.random() * 100, Math.random() * 5);
            }
        };
        this.drawPermanentGlitches = () => {
            this.permanentGlitches.forEach((element) => {
                ...
                rect(element.x + randomStart, element.y, element.w + randomStart, element.h);
                if (random(0, 1) > 0.95) {
                    fill(20);
                    rect(element.x + random(-element.w / 2, element.w / 2), element.y, random(20, 60), element.h);
                }
                if (move) {
                    translate(-offset, -offset);
                }
        ...
        this.enterGlitchMode = () => {
            ...
            for (let i = 0; i < 350; i++) {
                ...
                rect(Math.floor(Math.random() * innerWidth), Math.floor(Math.random() * innerHeight), Math.floor(Math.random() * 50) + 1, Math.floor(Math.random() * 50) + 1);
