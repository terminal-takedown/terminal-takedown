...
class Command {
    constructor(text, x, terminalText) {
        this.posY = 0;
        this.posX = 0;
        this.text = '';
        this.terminalText = '';
    ...
    draw() {
        ...
        fill(255, 0, 0);
        text(Array.from(errorText)
            .map((c) => (c === ' ' ? '_' : c))
            .join(''), this.posX + textWidth(typedText), this.posY);
        fill(200);
        text(untypedText, this.posX + textWidth(typedText) + textWidth(errorText), this.posY);
    }
    destruct() {
        textSize(32);
        const particles = [];
        for (let i = 0; i < textWidth(this.text); i += MatrixParticle.size + 5) {
            particles.push(new MatrixParticle(this.posX + i, this.posY, [0, 255, 0]));
        }
        return particles;
    }
    resize() {
