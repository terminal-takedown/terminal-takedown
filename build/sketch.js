...
function getCommand() {
    return random(commands)
        .replaceAll('{randi}', Math.floor(random(1, 10 + (score + failCount) * 5)).toString())
...
function draw() {
    ...
    if (gameState === 'boot') {
        const factor = 0.5;
        const y = windowHeight / 2 - (windowHeight * factor) / 2 - windowHeight * 0.1;
        image(img, windowWidth / 2 - (windowHeight * factor) / 2, y, windowHeight * factor, windowHeight * factor);
        ...
        text(messageTop, windowWidth / 2 - textWidth(messageCommand) / 2, windowHeight / 2 - 40);
        text(messageCommand, windowWidth / 2 - textWidth(messageCommand) / 2, windowHeight / 2);
        if (random() > 0.97) {
            matrixParticles.push(new MatrixParticle(random(20, windowWidth - 20), random(0, windowHeight - 300), [0, 255, 0]));
        }
    }
        ...
        if (command !== null) {
            ...
            if (command.posY >
                ...
                for (let i = 0; i < failedText.length; i++) {
                    const particle = new CharParticle(failedText[i], textWidth(terminal.prompt) +
                        30 +
                        textWidth('a') * i +
                        10, windowHeight - 60);
        ...
        if (random() > 0.97) {
            matrixParticles.push(new MatrixParticle(random(20, windowWidth - 20), random(0, windowHeight - 300), [255, 0, 0]));
        }
    }
    terminal.draw();
    if (localStorage.getItem('debug') == 'true') {
        textSize(20);
        fill('green');
        text(Math.round(frameRate()).toString(), innerWidth - 25, innerHeight - 20, 50, 50);
    }
}
function keyTyped() {
    if (key === 'Enter') {
...
function queueNewCommand(specialTimeout = undefined) {
    setTimeout(() => {
        textSize(32);
